...
def get_global_cameras(verts, device, distance=5, position=(-5.0, 5.0, 0.0)):
    positions = torch.tensor([position]).repeat(len(verts), 1).to(device)
    targets = verts.mean(1).to(device)
    verts = verts.to(device)

    directions = (targets - positions).to(device)
    directions = (directions / torch.norm(directions, dim=-1).unsqueeze(-1) * distance).to(device)
    positions = (targets - directions).to(device)

    print("positions", positions.device)
    print("targets", targets.device)

    rotation = look_at_rotation(positions, targets, device=device).mT
