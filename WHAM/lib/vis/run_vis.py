...
def run_vis_on_demo(cfg, video, results, output_pth, smpl, vis_global=True):
    ...
    # Get all frame_ids that have results (only render frames with results)
    all_frame_ids = set()
    for _id, val in results.items():
        all_frame_ids.update(val['frame_ids'])
    all_frame_ids = sorted(list(all_frame_ids))
    n_frames_to_render = len(all_frame_ids)
    
    writer = imageio.get_writer(
        osp.join(output_pth, 'output.mp4'), 
        fps=fps, mode='I', format='FFMPEG', macro_block_size=1
    )
    bar = Bar('Rendering results ...', fill='#', max=n_frames_to_render)
    
    _global_R, _global_T = None, None
    # run rendering - only process frames that have results
