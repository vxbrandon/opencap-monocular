...
class Camera:
    ...
    def __init__(self, id=None):
        ...
        self.Kundistortion = np.array(
            []
        )  # could be altered based on K using set_undistorted_view(alpha)
        ...
        self.calibration_type = (
            "standard"  # other possible values: bouguet, kannala, division, opencv
        )
        self.update_P()
    ...
    def save(self, filename):
        ...
        if self.calibration_type == "bouguet":
            ...
            or self.calibration_type == "opencv_fisheye"
        ):
    ...
    def load(self, filename):
        ...
        if (
            ...
            and "R" not in data
            and "t" not in data
            and "size_px" not in data
            and "calibration_type" not in data
            and "Kundistortion" not in data
        ):
    ...
    def set_K_elements(self, u0_px, v0_px, f=1, theta_rad=math.pi / 2, a=1):
        ...
        self.K = np.array(
            [
                [f, -f * 1 / math.tan(theta_rad), u0_px],
    ...
    def undistort_image(self, img, Kundistortion=None):
        ...
        if self.calibration_type == "opencv":
            return cv2.undistort(
                img, self.K, self.opencv_dist_coeff, newCameraMatrix=Kundistortion
            )
        elif self.calibration_type == "opencv_fisheye":
            return cv2.fisheye.undistortImage(
                img, self.K, self.opencv_dist_coeff, Knew=Kundistortion
            )
        else:
            ...
            if img.ndim == 2:
                return griddata(
                    y_l.T, img.ravel(), (xx, yy), fill_value=0, method="linear"
                )
            else:
                channels = [
                    griddata(
    ...
    def undistort(self, distorted_image_coords, Kundistortion=None):
        ...
        if self.calibration_type == "division":
            ...
            undistorted_image_coords = p2e(
                A.dot(
                    e2p(
                        self._undistort_division(
                            p2e(Ainv.dot(e2p(distorted_image_coords)))
                        )
                    )
                )
            )
        elif self.calibration_type == "opencv":
            undistorted_image_coords = (
                cv2.undistortPoints(
    ...
    def distort(self, undistorted_image_coords, Kundistortion=None):
        ...
        if self.calibration_type == "division":
            A = self.get_A(Kundistortion)
            Ainv = np.linalg.inv(A)
            distorted_image_coords = p2e(
                A.dot(
                    e2p(
                        self._distort_division(
                            p2e(Ainv.dot(e2p(undistorted_image_coords)))
                        )
                    )
                )
            )
        elif self.calibration_type == "opencv":
            undistorted_image_coords_norm = (
                undistorted_image_coords - column(Kundistortion[0:2, 2])
            ) / column(Kundistortion.diagonal()[0:2])
            undistorted_image_coords_3d = np.vstack(
                (
                    undistorted_image_coords_norm,
                    np.zeros((1, undistorted_image_coords.shape[1])),
                )
            )
            distorted_image_coords, _ = cv2.projectPoints(
                undistorted_image_coords_3d.T,
                (0, 0, 0),
                (0, 0, 0),
                self.K,
                self.opencv_dist_coeff,
            ...
            undistorted_image_coords = p2e(
                np.matmul(np.linalg.inv(Kundistortion), e2p(undistorted_image_coords))
            )
            distorted_image_coords = (
                cv2.fisheye.distortPoints(
                    undistorted_image_coords.T.reshape((1, -1, 2)),
                    self.K,
                    self.opencv_dist_coeff,
                )
                .reshape(-1, 2)
    ...
    def _distort_bouguet(self, undistorted_centered_image_coord):
        ...
        dx = np.array(
            [
                2 * kc[2] * x * y + kc[3] * (r_squared + 2 * x**2),
                kc[2] * (r_squared + 2 * y**2) + 2 * kc[3] * x * y,
            ]
        )
        distorted = (
            1 + kc[0] * r_squared + kc[1] * r_squared**2 + kc[4] * r_squared**3
    ...
    def _distort_tsai(self, metric_image_coord):
        ...
        t = 1.0 / 3 * np.arctan2(delta_sqrt, r[~positive_mask])
    ...
    def _undistort_division(self, z_r):
        ...
        return (
            (1 - self.division_lambda)
            / (1 - self.division_lambda * np.sum(z_r**2, axis=0) / self.division_z_n**2)
            * z_r
        )
    ...
    def world_to_image(self, world):
        ...
        if (
            ...
            if self.calibration_type == "opencv":
                distorted_image_coords = (
                    cv2.projectPoints(
                        world.T, self.R, self.t, self.K, self.opencv_dist_coeff
                    )[0]
                    .reshape(-1, 2)
                    .T
                )
        ...
        if self.calibration_type == "bouguet":
            ...
            # see [1, page 54]
            return self.get_A().dot(
                e2p(self._distort_division(p2e(self.get_k0().dot(camera_coords))))
            )
        else:
            xy = camera_coords[0:2, :]
            z = camera_coords[2, :]
    ...
    def image_to_world(self, image_px, z):
        ...
        tmpP = np.hstack(
            (self.P[:, [0, 1]], self.P[:, 2, np.newaxis] * z + self.P[:, 3, np.newaxis])
        )
    ...
    def get_view_matrix(self, alpha):
        ...
        if alpha == -1:
            ...
            Kundistortion, _ = cv2.getOptimalNewCameraMatrix(
                self.K, self.opencv_dist_coeff, tuple(self.size_px), alpha
            )
        elif self.calibration_type == "opencv_fisheye":
            Kundistortion = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(
                self.K,
                self.opencv_dist_coeff,
                tuple(self.size_px),
                self.R,
    ...
    def plot_world_points(self, points, plot_style, label=None, solve_visibility=True):
        """
        ...
        if not solve_visibility or np.all(self.is_visible(image_points_px)):
            plt.plot(image_points_px[0, :], image_points_px[1, :], plot_style)
            if label:
...
def nview_linear_triangulation(cameras, correspondences, weights=None):
    ...
    def _construct_D_block(P, uv, w=1):
        ...
        return w * np.vstack((uv[0] * P[2, :] - P[0, :], uv[1] * P[2, :] - P[1, :]))
    ...
    if weights is None:
        ...
        w = [
            np.nan_to_num(wi, nan=0.5) for wi in weights
        ]  # turns nan confidences into 0.5

    D = np.zeros((len(cameras) * 2, 4))
    for cam_idx, cam, uv in zip(range(len(cameras)), cameras, correspondences.T):
        D[cam_idx * 2 : cam_idx * 2 + 2, :] = _construct_D_block(
            cam.P, uv, w=w[cam_idx]
        )
    Q = D.T.dot(D)
    ...
    if np.count_nonzero(weights) < 2:
        ...
        if all(np.isnan(weightArray[weightArray != 0])):
            conf = 0.5  # nans get 0.5 confidence
        else:
...
def rotateIntrinsics(CamParams, rotation, imageSize=None):
    ...
    if rotation != 90:
        ...
        # Handle both old format (with intrinsicMat) and new format (with fx, fy, cx, cy)
        if "intrinsicMat" in originalCamParams:
            logger.info("Intrinsics are in old format")
            # Old format
            ...
            # Ensure intrinsicMat exists in CamParams for modification
            if "intrinsicMat" not in CamParams:
                CamParams["intrinsicMat"] = originalCamParams["intrinsicMat"].copy()
        elif "fx" in originalCamParams:
            logger.info("Intrinsics are in new format")
            # New format - reconstruct intrinsicMat
            fx = originalCamParams["fx"]
            fy = originalCamParams["fy"]
            px = originalCamParams["cx"]
            py = originalCamParams["cy"]
            # Create intrinsicMat if it doesn't exist
            if "intrinsicMat" not in CamParams:
                logger.info("Creating intrinsicMat in new format")
                CamParams["intrinsicMat"] = np.array(
                    [[fx, 0, px], [0, fy, py], [0, 0, 1]]
                )
        else:
            raise KeyError(
                "Intrinsics must contain either 'intrinsicMat' or 'fx', 'fy', 'cx', 'cy'"
            )

        # Get image size - check multiple sources
        if "imageSize" in originalCamParams:
            # Handle both 2x1 array format [[height], [width]] and 1D array formats
            # Original format: imageSize[0] = height (ly), imageSize[1] = width (lx)
            imageSize_arr = np.array(originalCamParams["imageSize"])
            if imageSize_arr.ndim == 2 and imageSize_arr.shape[1] == 1:
                # 2x1 array format: [[height], [width]]
                ly = float(imageSize_arr[0, 0])  # height (first element)
                lx = float(imageSize_arr[1, 0])  # width (second element)
            else:
                # 1D array format: [height, width] (matching original code behavior)
                ly = float(imageSize_arr[0])  # height
                lx = float(imageSize_arr[1])  # width
            if "imageSize" not in CamParams:
                logger.info("Creating imageSize in new format")
                CamParams["imageSize"] = originalCamParams["imageSize"].copy()
        elif imageSize is not None:
            # log it
            logger.info(f"imageSize: {imageSize}")
            # imageSize passed as parameter: portrait (pre-rotation) dimensions
            # [H_portrait, W_portrait] — NOT the landscape video dimensions.
            # For a landscape video: H_portrait=W_landscape, W_portrait=H_landscape.
            if isinstance(imageSize, (list, tuple, np.ndarray)):
                ly, lx = imageSize[0], imageSize[1]
            else:
                raise ValueError(
                    "imageSize must be a list, tuple, or numpy array [H_portrait, W_portrait]"
                )
            CamParams["imageSize"] = np.array([ly, lx])
        else:
            raise KeyError(
                "imageSize is required for rotation. Provide it in CamParams or as a parameter."
            )
        ...
        if rotation == 0:  # leaning left
            ...
            # Update individual keys if they exist
            if "fx" in CamParams:
                logger.info("Updating fx, fy, cx, cy in new format")
                CamParams["fx"] = fy
                CamParams["fy"] = fx
                CamParams["cx"] = py
                CamParams["cy"] = lx - px
        ...
        if "imageSize" in CamParams:
            ly_rotated, lx_rotated = (
                CamParams["imageSize"][0],
                CamParams["imageSize"][1],
            )
            if rotation in [0, 180] and ly_rotated == lx_rotated:
                logger.warning(
                    f"After {rotation}° rotation, imageSize has equal dimensions [{ly_rotated}, {lx_rotated}]. "
                    "This may indicate an issue with the rotation transformation."
                )
