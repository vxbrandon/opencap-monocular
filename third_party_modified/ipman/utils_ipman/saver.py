    ...
    def save_checkpoint(
        ...
        checkpoint_filename = os.path.abspath(
            os.path.join(
                ...
                + f"_epoch{epoch}_stepcount_{total_step_count}.pt",
            )
        )
        checkpoint_filename = os.path.abspath(
            os.path.join(
        ...
        # alt_dir = '/ps/project/common/renderpeople_initialfit/4Priyanka/spin_ft_on_agora/logs/agora_train_full_spinfix/checkpoints'
        # alt_checkpoint_filename = os.path.abspath(os.path.join(alt_dir, timestamp.strftime('%Y_%m_%d-%H_%M_%S') + f'_epoch{epoch}.pt'))
        checkpoint = {}
        for model in models:
        ...
        # torch.save(checkpoint, alt_checkpoint_filename)
        return
