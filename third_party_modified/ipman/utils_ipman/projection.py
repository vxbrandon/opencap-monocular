...
def plot_3d_andCam(p3d, camWorld):
    ...
    max_range = np.max(
        np.array(
            [
                np.max(p3d[j, 0]) + np.min(p3d[j, 0]),
                np.max(p3d[j, 1]) + np.min(p3d[j, 1]),
                np.max(p3d[j, 2]) + np.min(p3d[j, 2]),
            ]
        )
    )
...
def project_2d(
    args, df, i, pNum, joints3d, imgPath, imgHeight, imgWidth, constants, meanPose=False
):
    if args.scene3d:
    ...
    if meanPose:
        ...
        trans3d = [
            df.iloc[i]["XLocal"][pNum],
            df.iloc[i]["YLocal"][pNum],
            df.iloc[i]["ZLocal"][pNum],
        ]

    if not (args.scene3d):
        gt2d, gt3d_camCoord, valid, valJoints = project2d_hdri(
            joints3d,
            focalLength=constants["focalLength"],
            trans3d=trans3d,
        ...
        gt2d, gt3d_camCoord, valid, valJoints = project2d_world(
            joints3d,
            focalLength=constants["focalLength"],
            trans3d=trans3d,
            dslr_sens_width=constants["dslr_sens_width"],
...
def project2d_hdri(
    ...
    camMat = np.array([[focalLength_x, 0, cx], [0, focalLength_y, cy], [0, 0, 1]])
    ...
    camera_rotationMatrix, _ = cv2.Rodrigues(
        np.array([constants["camPitch"] / 180 * np.pi, 0.0, 0.0]).reshape(3, 1)
    )
    ...
    #
    RT = np.concatenate((np.diag([1.0, 1.0, 1.0]), np.zeros((3, 1))), axis=1)
    j2d = np.zeros((j3d_new.shape[0], 2))
    for i in range(j3d_new.shape[0]):
        j2d[i, :] = project_point(
            np.concatenate([j3d_new[i, :], np.array([1])]), RT, camMat
        )
...
def rotation_unreal2cv(pitch, yaw, roll):
    ...
    rotmat = batch_euler2matrix(
        torch.tensor([[-pitch / 180 * np.pi, -yaw / 180 * np.pi, -roll / 180 * np.pi]])
    )[0].numpy()
    return rotmat
...
def project2d_world(
    ...
    for i in range(j3d_new.shape[0]):
        j2d[i, :] = project_point(
            np.concatenate([j3d_new[i, :], np.array([1])]), RT, cam_intrinsics
        )
...
def checkValidGT(
    j2d, resx, resy, threshold=0.5, return_val_joints=False, numJointsToCheck=22
):
